...
const certifications = Array.from(new Set(products.map((p) => p.certification))).sort();
...
function ProductCard({ product, index }: { product: Product; index: number }) {
  return (
    <motion.article
      ...
      whileHover={{ y: -6 }}
      className="group overflow-hidden rounded-2xl border border-border bg-card p-3 shadow-soft transition-shadow duration-300 hover:shadow-lift"
    >
      <div className="relative overflow-hidden rounded-xl bg-muted">
        ...
        />
        {!product.available ? (
          <span className="absolute inset-x-3 top-3 rounded-full bg-card/90 px-3 py-1 text-center text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
            Rupture de stock
          </span>
        ) : null}
      </div>
      ...
      <div className="px-2 pb-1 pt-4">
        <div className="flex items-center gap-1.5 text-[11px] uppercase tracking-wider text-muted-foreground">
          <MapPin className="h-3.5 w-3.5 text-leaf" aria-hidden />
          {product.origin}
        </div>
        <h3 className="mt-2 font-display text-base font-semibold leading-snug">{product.name}</h3>
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-gradient-leaf text-primary-foreground transition-transform duration-200 hover:scale-110 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:scale-100"
          >
      ...
      <main>
        ...
        <section className="bg-cream">
          <div className="mx-auto grid max-w-7xl items-center gap-12 px-5 py-16 md:py-24 lg:grid-cols-2">
            ...
            >
              <p className="font-script text-3xl text-leaf">Bienvenue chez Rekany</p>
              <h1 className="mt-3 font-display text-4xl font-extrabold leading-[1.08] md:text-6xl text-foreground">
                Bon Produit
                <br />
                <span className="text-primary">Bonne Santé</span>
                <br />
                Bonne Vie
              </h1>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <a
                  href="#catalogue"
                  className="inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-soft transition-transform duration-200 hover:scale-105"
                >
                  Commander
                ...
                <Link
                  to="/"
                  className="inline-flex items-center gap-2 rounded-lg border border-primary/30 px-6 py-3 text-sm font-semibold text-primary transition-colors hover:bg-primary/5"
                >
                  Notre histoire
            ...
            >
              <div className="pointer-events-none absolute -inset-6 rounded-full bg-leaf/10 blur-3xl" />
              <div className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-white/70 p-3 shadow-[0_25px_70px_-20px_rgba(31,85,49,0.35)] backdrop-blur-sm">
                <motion.img
                  src={heroBowl}
        ...
        <section className="border-y border-border bg-card">
          <div className="mx-auto grid max-w-7xl gap-8 px-5 py-10 sm:grid-cols-2 lg:grid-cols-4">
            {highlights.map((h, i) => (
              ...
              >
                <span className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-mint">
                  <h.icon className="h-5 w-5 text-primary" aria-hidden />
                </span>
                <div className="min-w-0">
                  <h2 className="truncate font-display text-sm font-semibold text-foreground">{h.title}</h2>
        ...
        <section id="catalogue" className="mx-auto max-w-7xl px-5 py-16 md:py-20">
          <p className="font-script text-2xl text-leaf">Nos produits</p>
          <div className="mt-1 flex flex-wrap items-end justify-between gap-4">
            <h2 className="font-display text-3xl font-extrabold md:text-4xl text-foreground">
              Notre <span className="text-primary">Catalogue Bio</span>
            </h2>
              ...
              <button
                ...
                className={`rounded-full px-5 py-2 text-sm font-medium transition-colors ${
                  category === c
                    ...
                    : "bg-mint text-primary hover:bg-primary/10"
                }`}
              >
                {c}
              </button>
              ...
              <div className="lg:col-span-2">
                ...
                <div className="relative mt-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-input bg-card py-2.5 pl-11 pr-4 text-sm outline-none transition focus-visible:ring-2 focus-visible:ring-ring text-foreground"
                  />
                </div>
              </div>
        ...
        <section className="mx-auto max-w-7xl px-5 pb-20">
          <motion.div
            ...
            className="overflow-hidden rounded-3xl bg-mint px-8 py-12 md:px-14"
          >
            <h2 className="font-display text-2xl font-extrabold md:text-4xl text-foreground">
              Commandez <span className="text-primary">en gros</span>
            </h2>
            ...
            <a
              ...
              className="mt-7 inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground transition-transform duration-200 hover:scale-105"
            >
              Demander un devis
              <ArrowRight className="h-4 w-4" aria-hidden />
            </a>
  ...
  return (
    <div>
      ...
      <select
        ...
        className="mt-2 w-full rounded-lg border border-input bg-card px-4 py-2.5 text-sm outline-none transition focus-visible:ring-2 focus-visible:ring-ring text-foreground"
      >
        {options.map((o) => (
